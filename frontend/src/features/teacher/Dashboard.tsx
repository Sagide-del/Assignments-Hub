...
// Covers the "JSON Upload" assignment-creation method from the brief for
// real — it calls the actual POST /assignments/validate and
// POST /assignments/from-json endpoints. The manual question-builder lives
// at /teacher/assignments/new (CreateAssignment.tsx). There's still no AI
// assistant button here — no AI-generation endpoint exists on the backend
// (see ROADMAP.md) — no fake button that quietly does nothing.
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link to="/teacher/assignments/new" className="px-3 py-2 text-sm rounded border border-gray-300">
            + Manual assignment
          </Link>
          <Link to="/teacher/marking" className="px-3 py-2 text-sm rounded bg-brand text-white">
            Marking
          </Link>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
        <h2 className="font-medium text-sm">Upload Assignment (JSON)</h2>
        <textarea
          value={jsonText}
        ...
        {validation && (
          <div className={`text-sm ${validation.valid ? 'text-green-700' : 'text-red-600'}`}>
          ...
          {(assignments ?? []).map((a) => (
            <li key={a.id} className="px-4 py-3 flex items-center justify-between text-sm">
              <div>
                ...
                <p className="text-gray-500">{a.subject} · Grade {a.grade}</p>
              </div>
              <span className={`text-xs px-2 py-1 rounded ${a.isPublished ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
